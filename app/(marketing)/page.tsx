...
/* ─── PORTFOLIO ─────────────────────────────────────────────────── */
const PORTFOLIO = [
  { client: 'Pavel Novák Instalace', obor: 'Instalatér', city: 'Praha', result: '+340% kontaktů', color: 'from-[#285570] to-[#1a3446]' },
  { client: 'Zubní ordinace Procházková', obor: 'Zubař', city: 'Brno', result: '70% nových pacientů', color: 'from-stone-400 to-stone-600' },
  { client: 'Kavárna Na Rohu', obor: 'Kavárna', city: 'Jihlava', result: '+40% rezervací', color: 'from-[#8fa8b8] to-[#285570]' },
];
    ...
    <div ref={outerRef} style={{ background: BG, padding: '220px 24px' }}>
      <div style={{ maxWidth: '1080px', margin: '0 auto' }}>
        ...
        <motion.div animate={{ opacity: inView ? 1 : 0, x: inView ? 0 : 80 }} transition={E(0)} className="mb-10">
          <p style={{ fontSize: '11px', fontWeight: 600, color: 'rgba(40,85,112,0.55)', textTransform: 'uppercase', letterSpacing: '0.16em', marginBottom: '14px' }}>Naše práce</p>
          <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
            <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(26px,3.8vw,50px)', fontWeight: 300, color: '#1a2e3d', letterSpacing: '-0.03em', lineHeight: 1.1, margin: 0 }}>
              Výsledky,<br />které mluví za sebe
            </h2>
            <Link href="/portfolio" style={{ fontSize: '13px', fontWeight: 600, color: '#cbcac7', textDecoration: 'none', whiteSpace: 'nowrap', transition: 'color .15s' }}
              onMouseEnter={e => (e.currentTarget.style.color = ACCENT)}
              onMouseLeave={e => (e.currentTarget.style.color = '#cbcac7')}
            >Celé portfolio →</Link>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {PORTFOLIO.map((p, i) => (
            <motion.div key={p.client}
              ...
              style={{ overflow: 'hidden', borderRadius: '18px', border: '1px solid #e3ded7', boxShadow: '0 2px 16px rgba(40,85,112,0.05)' }}>
              <div className={`h-36 bg-gradient-to-br ${p.color} relative overflow-hidden`}>
                <div className="absolute inset-3 rounded-lg" style={{ background: 'rgba(255,255,255,0.12)', border: '1px solid rgba(255,255,255,0.2)' }}>
                  <div className="flex items-center gap-1.5 px-3 py-2" style={{ borderBottom: '1px solid rgba(255,255,255,0.15)' }}>
                    {[0,1,2].map(j => <div key={j} className="w-2 h-2 rounded-full" style={{ background: 'rgba(255,255,255,0.35)' }} />)}
                    <div className="flex-1 mx-2 h-2.5 rounded" style={{ background: 'rgba(255,255,255,0.2)' }} />
                  </div>
                  <div className="p-3 space-y-1.5">
                    <div className="h-2.5 rounded w-3/4" style={{ background: 'rgba(255,255,255,0.3)' }} />
                    <div className="h-2 rounded w-full" style={{ background: 'rgba(255,255,255,0.18)' }} />
                    <div className="h-2 rounded w-5/6" style={{ background: 'rgba(255,255,255,0.18)' }} />
                  </div>
                </div>
              </div>
              <div style={{ padding: '20px 24px', background: '#fff' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '6px' }}>
                  <span style={{ fontSize: '11px', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em', color: '#cbcac7' }}>{p.obor}</span>
                  <span style={{ color: '#e3ded7' }}>·</span>
                  <span style={{ fontSize: '11px', color: '#cbcac7' }}>{p.city}</span>
                </div>
                <p style={{ fontSize: '14px', fontWeight: 600, color: '#1a2e3d', marginBottom: '10px' }}>{p.client}</p>
                <span style={{ fontSize: '12px', fontWeight: 600, color: ACCENT, background: 'rgba(40,85,112,0.07)', border: '1px solid rgba(40,85,112,0.14)', borderRadius: '20px', padding: '4px 12px', display: 'inline-block' }}>
                  {p.result}
                </span>
              </div>
            </motion.div>
  ...
  return (
    <div ref={outerRef} style={{ background: BG2, padding: '220px 24px' }}>
      <div style={{ maxWidth: '1080px', margin: '0 auto' }}>
        ...
        <motion.div animate={{ opacity: inView ? 1 : 0, x: inView ? 0 : -80 }} transition={E(0)} className="mb-10">
          <p style={{ fontSize: '11px', fontWeight: 600, color: 'rgba(40,85,112,0.55)', textTransform: 'uppercase', letterSpacing: '0.16em', marginBottom: '14px' }}>Reference</p>
          <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
            <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(26px,3.8vw,50px)', fontWeight: 300, color: '#1a2e3d', letterSpacing: '-0.03em', lineHeight: 1.1, margin: 0 }}>
              Co říkají klienti
            </h2>
            <Link href="/reference" style={{ fontSize: '13px', fontWeight: 600, color: '#cbcac7', textDecoration: 'none', whiteSpace: 'nowrap', transition: 'color .15s' }}
              onMouseEnter={e => (e.currentTarget.style.color = ACCENT)}
              onMouseLeave={e => (e.currentTarget.style.color = '#cbcac7')}
          ...
          {TESTIMONIALS.map((t, i) => (
            <motion.div key={t.name}
              ...
              <div style={{ display: 'flex', gap: '2px', marginBottom: '18px' }}>
                {[1,2,3,4,5].map(s => (
                  <svg key={s} width="12" height="12" viewBox="0 0 24 24" fill="#285570">
                    <polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2" />
                  </svg>
                ))}
              </div>
              <p style={{ fontSize: '13px', lineHeight: 1.7, color: '#6b6560', marginBottom: '24px' }}>&ldquo;{t.quote}&rdquo;</p>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                <div style={{ width: '36px', height: '36px', borderRadius: '50%', background: 'rgba(40,85,112,0.08)', border: `1px solid rgba(40,85,112,0.15)`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '13px', fontWeight: 700, color: ACCENT, flexShrink: 0 }}>
                  {t.initial}
                </div>
                <div>
                  <p style={{ fontSize: '13px', fontWeight: 600, color: '#1a2e3d', marginBottom: '1px' }}>{t.name}</p>
                  <p style={{ fontSize: '11px', color: '#cbcac7' }}>{t.role}</p>
                </div>
              </div>
            </motion.div>
          ))}
